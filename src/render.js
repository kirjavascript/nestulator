...
exports.renderSprites = exports.renderBG = void 0;
...
function renderBG(nes) {
    const { bus, VRAM, CHR } = nes;
    if (!bus.backgroundDirty)
        return;
    ...
    background.style.backgroundColor = colors_1.paletteHex[palettes[0][0]];
    ...
    for (let y = 0; y < background.height / 8; y++) {
        for (let x = 0; x < background.width / 8; x++) {
            ...
            const attrIndex = Math.floor(x / 4) + (Math.floor(y / 4) * 8) + 0x23c0;
            ...
            pixels.forEach((pixel, i) => {
                if (pixel !== 0) { // can ignore transparent pixels
                    if (greyscale) {
                        ...
                        const [r, g, b] = colors_1.paletteRGB[palette[pixel]];
